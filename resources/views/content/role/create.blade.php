    ...
    <div class="modal-dialog modal-lg modal-dialog-centered modal-add-new-role">
        <div class="modal-content p-3 p-md-5">
            ...
            <div class="modal-body">
                ...
                <!-- Add role form -->
                <form class="row g-3" action="{{ route('user.role.store') }}" method="POST">
                    @csrf
                    <div class="col-12 mb-4">
                        <label class="form-label" for="name">Role Name</label>
                        <input type="text" id="name" class="form-control @error('name') is-invalid @enderror"
                            placeholder="Enter a role name" name="name" value="{{ old('name') }}" />
                        @error('name')
                    ...
                    @error('permission')
                        <div class="alert alert-solid-danger" role="alert">
                            ...
                            <span>{{ $errors->first('permission') }}</span>
                        </div>
                    @enderror
                    <div class="col-12">
                                                        ...
                                                        @if (in_array($act . ' ' . $module, $permission))
                                                            <div class="form-check me-3 me-lg-5">
                                                                <input class="form-check-input" type="checkbox"
                                                                    ...
                                                                    value="{{ $perm_id }}"
                                                                    @checked(in_array($perm_id, old('permission') ?? [])) />
