...
@section('content')
    ...
    @endif
    <h4 class="py-3 breadcrumb-wrapper mb-2">Admin / Liste des produits</h4>

    <!-- Users List Table -->
    <h4 class="py-3 breadcrumb-wrapper mb-2"></h4>
    <div class="card">
        <div class="card-header">
            <div class="row">
                <form action="{{ route('product.index') }}" method="GET" class="d-sm-flex d-md-flex  col-9 ">
                    ...
                    <div class="col-sm-2 m-3">
                        ...
                        <a href="{{ route('product.index') }}" class="text-dark mx-3">Reset</a>
                    </div>
                </form>
                <div class="col-sm-3">
                    <a href="{{ route('product.create') }}" class="btn btn-primary"><i class="bx bx-plus me-0 me-lg-2"></i> Ajouter
                        un
                        produit</a>
                </div>
                
            </div>
            <span class="text-muted col-lg-12 mx-3 mt-n4"><b>{{ $products->total() }} result(s)</b></span>
                ...
                <tbody>
                    @foreach ($products as $product)
                        <tr id="row_19">
                            ...
                            <td>
                                <span class="badge-basic-primary-text">
                                    @if ($product?->category?->parent)
                                        ...
                                        @endphp
                                        <span class="badge-basic-primary-text">
                                            &rarr; {{ $product->category->parent->name }}
                                        </span>
                                        <span class="badge-basic-primary-text">
                                            &rarr; {{ $product->category->name }}
                                        </span>
                                    @else
                            ...
                            <td>
                                <span class="badge-basic-primary-text">
                                    {{ $product->quantity }} {{ $product->unit_mesure }}
                                </span>
                            </td>
                            <td>
                                ...
                                @endif
                                {{-- <span class="badge-basic-primary-text">
                                    {{ $product->status_pro ? 'Active' : 'Inactive' }}
                                </span> --}}
                            </td>
                            <td class="action">
                                <div class="dropdown">
